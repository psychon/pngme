...
impl Png {
    ...
    pub fn append_chunk(&mut self, chunk: Chunk) {
        assert_eq!(
            ...
            self.chunks
                .iter()
                .find(|c| c.chunk_type() == chunk.chunk_type())
            );
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        let index = self.chunks.iter()
            ...
            .map(|(index, _)| index)
            .ok_or_else(|| anyhow!("No chunk {} found", chunk_type))?;
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        let header = value.get(..Self::STANDARD_HEADER.len());
        if header != Some(&Self::STANDARD_HEADER) {
            return Err(anyhow!("Incorrect file header"));
        }

        let mut data = &value[Self::STANDARD_HEADER.len()..];
        let mut chunks = Vec::new();
        while !data.is_empty() {
            let (chunk, remaining) = Chunk::parse_next(data)?;
            chunks.push(chunk);
            data = remaining;
        }
        Ok(Self { chunks })
...
mod tests {
    ...
    fn test_valid_from_bytes() {
        ...
        assert_eq!(png.chunks(), &testing_chunks()[..]);
    }
